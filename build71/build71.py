# -*- coding: utf-8 -*-
"""第71回(2025) ビルド: 問題/正答/別冊画像 -> CSV・images71.json・埋め込みJS。
素材:
  am_parsed.json / pm_parsed.json … pdftext自動抽出（要整形）
  overrides.json … 斜体ラテン語・組合せ・表/図問題の手修正
  /tmp/k71/ans71.json … 正答表(ANS A/B)
  /tmp/k71/eam/noNN.png, epm/noNN.png … 別冊画像(No.=ページ-4, A/Bは同一ページ)
...
def main():
    ...
    ans = json.load(open(os.path.join(TMP,'ans71.json'),encoding='utf-8'))
    ...
    with open(os.path.join(HERE,'..','臨床検査技師_第71回.csv'),'w',encoding='utf-8-sig',newline='') as f:
