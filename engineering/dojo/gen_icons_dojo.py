# -*- coding: utf-8 -*-
"""過去問道場（臨床工学技士＝コウゼミ）アプリのアイコン生成 → docs/kouzemi-dojo/icon-*.png。
グリーン系グラデーション地に白文字「過去問／道場」。マスカブルは中央安全域に収める。
（ケンゼミ docs/dojo/・ホウゼミ docs/houzemi-dojo/ には触れない）"""
import os
...
BOT = (6, 78, 38)       # green-800 #166534-ish
...
def make(size, scale, rounded, out):
    ...
    cx = size // 2
    u = size * scale
    f_top = ImageFont.truetype(FONT, int(u * 0.190))
    f_big = ImageFont.truetype(FONT, int(u * 0.360))
    block_top = size * (0.5 - scale * 0.40)
    # 上段「過去問」(字間ひろめ)
    h1 = ctext(d, cx, block_top, '過 去 問', f_top, (187, 247, 208, 255))
    # 中段「道場」(大・白・影)
    y2 = block_top + h1 + u * 0.085
    sh = Image.new('RGBA', img.size, (0, 0, 0, 0)); ds = ImageDraw.Draw(sh)
    ctext(ds, cx + max(1, int(size * 0.006)), y2 + max(1, int(size * 0.006)), '道場', f_big, (4, 40, 18, 150))
...
print('icons -> docs/kouzemi-dojo/ (文字ロゴ 過去問/道場・グリーン)')
