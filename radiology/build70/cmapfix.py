# -*- coding: utf-8 -*-
"""第70回(tp180511-06) cmap復元。数字共通、括弧グリフ固有。
数字 \\x02->2 \\x04->1 \\x05->5 \\x06->3 \\x07->4 guj->2 / 庵x 袷+ ï梢 安- 暗=
括弧: 午前 \\x94->[ \\x95->] / Ï-><  Ô->> ; 午後 ä->[ å->] / \\x87-><  \\x90->>
脱字: »->腿 Ç->梢(午前) \\x17->穿(午後)。PUA(数式)除去。検証 平成30年2月22日(=2018,第70回)。
"""
import re, sys
...
def fix(text, side):
    ...
    s = s.replace('大»','大腿')
    if side == 'AM':
        s = s.replace('\x94','［').replace('\x95','］')
        s = s.replace('Ï','〈').replace('Ô','〉')
        s = s.replace('末Ç','末梢').replace('Ç','梢')
    else:
        s = s.replace('ä','［').replace('å','］')
        s = s.replace('\x87','〈').replace('\x90','〉')
        s = s.replace('\x17','穿')
...
if __name__ == '__main__':
    tmp = sys.argv[1] if len(sys.argv)>1 else '/tmp/k70'
