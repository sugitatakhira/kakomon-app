# -*- coding: utf-8 -*-
"""第68回(tp160411-04) cmap復元。100/100問。数字共通、括弧グリフ固有。
数字 \\x02->2 \\x04->1 \\x05->5 \\x06->3 \\x07->4 guj->2 / 庵x 袷+ ï梢 安- 暗=
括弧: 午前 \\x1C->[ \\x1D->] / k...l -> <...>(片仮名読み) ; 午後 X...[ -> [...]
脱字: \\x10->扁 Ö->腿 \\x11孔->穿孔 \\x11->0。PUA(数式)除去。検証 平成28年2月25日(=2016)。
"""
import re, sys
...
def fix(text, side):
    ...
    s = s.replace('\x10','扁').replace('Ö','腿')
    s = s.replace('\x11孔','穿孔').replace('\x11','0')
    if side == 'AM':
        s = s.replace('\x1c','［').replace('\x1d','］')
        s = re.sub(r'k([ぁ-ヶ・ー]{2,12}?)l', r'〈\1〉', s)
    else:
        s = re.sub(r'X([A-Za-z0-9μ/･・°℃Ω％% ]{1,8}?)\[', r'［\1］', s)
...
if __name__ == '__main__':
    tmp = sys.argv[1] if len(sys.argv)>1 else '/tmp/k68'
