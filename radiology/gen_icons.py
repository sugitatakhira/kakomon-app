# -*- coding: utf-8 -*-
"""ホウゼミ PWAアプリアイコン生成 → docs/houzemi/icon-*.png。文字ロゴ「ホウゼミ／過去問／3000」。
インディゴ系グラデーション地に白文字(ケンゼミ=ティールと別系統)。マスカブルは中央安全域に収める。"""
import os
...
BOT=(30,27,75)        # indigo-950 #1e1b4b
AMBER=(251,191,36)
...
def make(size, scale, rounded, out):
    ...
    h1=ctext(d, cx, block_top, 'ホ ウ ゼ ミ', f_top, (199,205,253,255))     # 上段(薄インディゴ)
    y2=block_top+h1+u*0.075
    sh=Image.new('RGBA',img.size,(0,0,0,0)); ds=ImageDraw.Draw(sh)
    ctext(ds, cx+max(1,int(size*0.006)), y2+max(1,int(size*0.006)), '過去問', f_big, (20,18,60,150))
    ...
    y3=ly+u*0.075
    ctext(d, cx, y3, '3000', f_db, AMBER+(255,))                          # 下段(アンバー)
...
print('icons -> docs/houzemi/ (文字ロゴ ホウゼミ/過去問/3000)')
