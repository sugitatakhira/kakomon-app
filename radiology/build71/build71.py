...
# 診療放射線技師 分野ブロック(午前・午後 共通。第71回の出題順から導出)。合計100。
...
def build_side(parsed, ov, ans, prefix_label, id_label, img_dir):
    ...
    for q in parsed:
        ...
        qid = "k71%s%s%03d" % (id_label, prefix_label, n)
...
def main():
    ...
    ans = json.load(open(os.path.join(HERE,'ans71.json'),encoding='utf-8'))
    ...
    json.dump(images, open(os.path.join(HERE,'..','images71.json'),'w',encoding='utf-8'), ensure_ascii=False)
    ...
    with open(os.path.join(HERE,'..','診療放射線技師_第71回.csv'),'w',encoding='utf-8-sig',newline='') as f:
        csv.writer(f).writerows(rows)

    json.dump(allq, open(os.path.join(HERE,'kokushi71.json'),'w',encoding='utf-8'), ensure_ascii=False)
