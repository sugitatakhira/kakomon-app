# -*- coding: utf-8 -*-
"""過去問道場（診療放射線技師＝ホウゼミ）アプリのアイコン生成 → docs/houzemi-dojo/icon-*.png。
インディゴ系グラデーション地に白文字「過去問／道場」。マスカブルは中央安全域に収める。
（リポジトリ直下 dojo/gen_icons_dojo.py のホウゼミ版。ケンゼミ docs/dojo/ には触れない）"""
import os
...
def make(size, scale, rounded, out):
    ...
    # 上段「過去問」(字間ひろめ)
    h1 = ctext(d, cx, block_top, '過 去 問', f_top, (210, 207, 250, 255))
    # 中段「道場」(大・白・影)
    y2 = block_top + h1 + u * 0.085
    sh = Image.new('RGBA', img.size, (0, 0, 0, 0)); ds = ImageDraw.Draw(sh)
    ctext(ds, cx + max(1, int(size * 0.006)), y2 + max(1, int(size * 0.006)), '道場', f_big, (25, 22, 70, 150))
...
print('icons -> docs/houzemi-dojo/ (文字ロゴ 過去問/道場・インディゴ)')
