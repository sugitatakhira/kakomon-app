# -*- coding: utf-8 -*-
"""過去問道場アプリのアイコン生成 → docs/dojo/icon-*.png。
ティール系グラデーション地に白文字「過去問／道場」。マスカブルは中央安全域に収める。
（既存 build71/gen_icons.py の道場版。docs/ のケンゼミDBアイコンには触れない）"""
import os
...
def make(size, scale, rounded, out):
    ...
    # 上段「過去問」(字間ひろめ)
    h1 = ctext(d, cx, block_top, '過 去 問', f_top, (199, 236, 234, 255))
    # 中段「道場」(大・白・影)
    y2 = block_top + h1 + u * 0.085
    sh = Image.new('RGBA', img.size, (0, 0, 0, 0)); ds = ImageDraw.Draw(sh)
    ctext(ds, cx + max(1, int(size * 0.006)), y2 + max(1, int(size * 0.006)), '道場', f_big, (0, 40, 55, 150))
...
print('icons -> docs/dojo/ (文字ロゴ 過去問/道場)')
