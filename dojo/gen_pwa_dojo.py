# -*- coding: utf-8 -*-
"""過去問道場(演習アプリ)のPWA化: 配布版 kakomon-dojo-all.html に manifest/Service Worker を
組み込み、docs/dojo/ 配下にデプロイ一式を出力する。
既存のケンゼミDB(docs/直下)とは別フォルダなので共存できる(同じGitHub Pagesサイトの /dojo/ で配信)。
SWのキャッシュ名は index.html の内容ハッシュに連動 → 変更があれば自動で新版に差し替わる。"""
import os, json, hashlib
HERE = os.path.dirname(__file__); ROOT = os.path.join(HERE, '..'); DOCS = os.path.join(ROOT, 'docs', 'dojo')
os.makedirs(DOCS, exist_ok=True)
html = open(os.path.join(ROOT, 'kakomon-dojo-all.html'), encoding='utf-8').read()

# 1) <head> に PWA メタ/manifest/アイコン を差し込む（テンプレ既存の theme-color はそのまま）
head_add = '''<title>臨床検査技師 過去問道場</title>
...
CACHE_NAME = "dojo-" + hashlib.md5(html.encode('utf-8')).hexdigest()[:12]
...
print("wrote docs/dojo/ : index.html(%.1fMB), manifest.webmanifest, sw.js" % (len(html) / 1e6))
