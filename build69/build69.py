# -*- coding: utf-8 -*-
"""第69回(2023) ビルド: 問題/正答/別冊画像 -> CSV・images69.json・埋め込みJS。
素材:
  am_parsed.json / pm_parsed.json … pdftext自動抽出（要整形）
  overrides.json … 斜体ラテン語・組合せ・表/図問題の手修正
  /tmp/k69/ans69.json … 正答表(ANS A/B)
  /tmp/k69/eam/noNN.png, epm/noNN.png … 別冊画像(No.=ページ-4, A/Bは同一ページ)
...
def build_side(parsed, ov, ans, prefix_label, id_label, img_dir, fig_map):
    ...
    for q in parsed:
        ...
        # 別冊 suffix from effective stem (override text if given, else parsed)
        ...
        qid = "k69%s%s%03d" % (id_label, prefix_label, n)
        item = {
            "id": qid,
...
def main():
    ...
    ans = json.load(open(os.path.join(TMP,'ans69.json'),encoding='utf-8'))
    ...
    # images69.json
    json.dump(images, open(os.path.join(HERE,'..','images69.json'),'w',encoding='utf-8'), ensure_ascii=False)
    ...
    with open(os.path.join(HERE,'..','臨床検査技師_第69回.csv'),'w',encoding='utf-8-sig',newline='') as f:
        csv.writer(f).writerows(rows)

    # embedded JS list (with images inlined into question objects, like 72)
    json.dump(allq, open(os.path.join(HERE,'kokushi69.json'),'w',encoding='utf-8'), ensure_ascii=False)
