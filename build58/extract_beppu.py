# -*- coding: utf-8 -*-
"""第58回 別冊抽出。午前 No.1-12=am_02(page=No+6)、午後 No.1-12=pm_02(page=No+6)。
心電図(午後No.3)は ROTATE_270。午前に回転図はなし。余白は自動トリム。"""
import os, subprocess
from PIL import Image, ImageChops
HERE=os.path.dirname(__file__); TMP='/tmp/k58'
AM=os.path.join(HERE,'am_02.pdf'); PM=os.path.join(HERE,'pm_02.pdf')
AM_PAGES={n:(AM,n+6) for n in range(1,13)}
PM_PAGES={n:(PM,n+6) for n in range(1,13)}
AM_ROT=set(); PM_ROT={3}
