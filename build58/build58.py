# -*- coding: utf-8 -*-
"""第58回(2012) ビルド: 問題/正答/別冊画像 -> CSV・images58.json・埋め込みJS(kokushi58.json)。
素材(すべて build68/ 配下・再現可):
  am_parsed.json / pm_parsed.json … 本文(cmapfix→parse)の自動抽出
  overrides.json … 組合せ「—」区切り・表/特殊グリフの手修正
  ans58.json … 正答表(A/B, 1始まり)
  /tmp/k58/eam/noNN.png, epm/noNN.png … 別冊画像(extract_beppu.pyで生成)
...
def clean_choice(s):
    # 組合せ問題の区切り(大きな空白=⟨GAP⟩)を — に。別冊参照は先にTAILで除去。
    s = TAIL.sub('', s)
    s = s.replace('⟨GAP⟩', ' — ')
    s = s.replace('‑','-')
    s = re.sub(r'\s+',' ', s).strip()
...
def build_side(parsed, ov, ans, prefix_label, id_label, img_dir):
    ...
    for q in parsed:
        ...
        qid = "k58%s%s%03d" % (id_label, prefix_label, n)
...
def main():
    ...
    ans = json.load(open(os.path.join(HERE,'ans58.json'),encoding='utf-8'))
    ...
    json.dump(images, open(os.path.join(HERE,'..','images58.json'),'w',encoding='utf-8'), ensure_ascii=False)
    ...
    with open(os.path.join(HERE,'..','臨床検査技師_第58回.csv'),'w',encoding='utf-8-sig',newline='') as f:
        csv.writer(f).writerows(rows)

    json.dump(allq, open(os.path.join(HERE,'kokushi58.json'),'w',encoding='utf-8'), ensure_ascii=False)
