...
- 実体は**単一HTML 3ファイル**（リポジトリ直下、ビルド不要・依存なし）：
  - `ecg-practice.html` … **ECG検定2級**想定・全51問（配色＝ティール緑系）
  - `ecg-practice-grade3.html` … **ECG検定3級**入門・全45問（配色＝青系）
  - `ecg-learn.html` … **波形の特徴を学ぶ学習ページ**（正常と上下2段で見比べ＋病期ボタンで波形が**ぬるぬるモーフィング**＋矢印注釈＋正常重ね表示。**3級入門/2級標準のレベル切替**つき。疾患は横スクロールタブ。配色＝藍×シアン）。モーフは mod パラメータ補間（同一疾患の mod 段階間）、リズム系はクロスフェード。
