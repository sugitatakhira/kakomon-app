# -*- coding: utf-8 -*-
"""第65回(2019) ビルド: 問題/正答/別冊画像 -> CSV・images65.json・埋め込みJS(kokushi65.json)。
素材(すべて build68/ 配下・再現可):
  am_parsed.json / pm_parsed.json … 本文(cmapfix→parse)の自動抽出
  overrides.json … 組合せ「—」区切り・表/特殊グリフの手修正
  ans65.json … 正答表(A/B, 1始まり)
  /tmp/k65/eam/noNN.png, epm/noNN.png … 別冊画像(extract_beppu.pyで生成)
...
def main():
    ...
    with open(os.path.join(HERE,'..','臨床検査技師_第65回.csv'),'w',encoding='utf-8-sig',newline='') as f:
