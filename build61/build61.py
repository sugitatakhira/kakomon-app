# -*- coding: utf-8 -*-
"""第61回(2015) ビルド: 問題/正答/別冊画像 -> CSV・images61.json・埋め込みJS(kokushi61.json)。
素材(すべて build68/ 配下・再現可):
  am_parsed.json / pm_parsed.json … 本文(cmapfix→parse)の自動抽出
  overrides.json … 組合せ「—」区切り・表/特殊グリフの手修正
  ans61.json … 正答表(A/B, 1始まり)
  /tmp/k61/eam/noNN.png, epm/noNN.png … 別冊画像(extract_beppu.pyで生成)
...
def build_side(parsed, ov, ans, prefix_label, id_label, img_dir):
    ...
    for q in parsed:
        ...
        qid = "k61%s%s%03d" % (id_label, prefix_label, n)
...
def main():
    ...
    ans = json.load(open(os.path.join(HERE,'ans61.json'),encoding='utf-8'))
    ...
    json.dump(images, open(os.path.join(HERE,'..','images61.json'),'w',encoding='utf-8'), ensure_ascii=False)
    ...
    with open(os.path.join(HERE,'..','臨床検査技師_第61回.csv'),'w',encoding='utf-8-sig',newline='') as f:
        csv.writer(f).writerows(rows)

    json.dump(allq, open(os.path.join(HERE,'kokushi61.json'),'w',encoding='utf-8'), ensure_ascii=False)
